...
const IndustryApps = () => {
    ...
    const fetchData = async () => {
        try {
            const { data } = await API.get('/homepage-sections/industry-applications');
            if (data.data) setItems(data.data);
        } catch (error) { toast.error('Failed to fetch Industry Applications'); }
    };
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (currentItem._id) {
                await API.put('/homepage-sections/' + 'industry-applications' + '/' + currentItem._id, currentItem);
                toast.success('Updated successfully');
            } else {
                await API.post('/homepage-sections/industry-applications', currentItem);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await API.delete('/homepage-sections/' + 'industry-applications' + '/' + id);
    ...
    return (
        <div className="p-6 max-w-6xl mx-auto animate-in fade-in duration-300">
            <div className="flex items-center justify-between mb-8">
                ...
                <button onClick={() => { setCurrentItem({"title":"","description":"","image":"","tag":""}); setModalOpen(true); }} className="flex items-center gap-2 bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition-colors shadow-sm font-medium">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200">
                                <th className="p-4 font-semibold text-slate-700 text-sm">Title</th><th className="p-4 font-semibold text-slate-700 text-sm">Description</th><th className="p-4 font-semibold text-slate-700 text-sm">Image URL</th><th className="p-4 font-semibold text-slate-700 text-sm">Tag (e.g. PRIMARY APPLICATION)</th>
                        ...
                        <tbody className="divide-y divide-slate-200">
                            {items.map((item) => (
                                <tr key={item._id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="p-4 text-sm text-slate-600">{item.title}</td><td className="p-4 text-sm text-slate-600">{item.description}</td><td className="p-4"><img src={item.image} alt="" className="w-16 h-12 object-cover rounded-md border border-slate-200" /></td><td className="p-4 text-sm text-slate-600">{item.tag}</td>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl w-full max-w-2xl shadow-xl flex flex-col max-h-[90vh]">
                        <div className="flex items-center justify-between p-6 border-b border-slate-100">
                            <h2 className="text-xl font-bold text-slate-800">{currentItem?._id ? 'Edit' : 'Add'} Industry Applications</h2>
                        ...
                        <div className="p-6 overflow-y-auto">
                            ...
                            <div className="space-y-1">
                                <label className="text-sm font-medium text-slate-700">Tag (e.g. PRIMARY APPLICATION)</label>
                                <input type="text" value={currentItem.tag || ''} onChange={e => setCurrentItem({...currentItem, tag: e.target.value})} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required />
                            </div>
                            </form>
                        </div>
